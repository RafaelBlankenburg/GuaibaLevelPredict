...
def preprocess_dataframe(df: pd.DataFrame, coluna_nivel: str):
    ...
    if has_date_column:
        ...
        df_features['dia_do_ano_sin'] = np.sin(2 * np.pi * df_original['data'].dt.dayofyear / 365)
        df_features['dia_do_ano_cos'] = np.cos(2 * np.pi * df_original['data'].dt.dayofyear / 365)
